...
//ParseGeneralSwaggerInfo Read web/main.go to get Swagger info
...
//ParseTypeDefinitions parse api type definintions
...
//ParseImportStatements parse import statements
...
//ParseAPIDescription parse aip description
...
//CheckRealPackagePath check package real path
...
//GetModelDefinition get model definition
...
//FindModelDefinition find model definintion
...
//ParserFileFilter parser file filter
...
//NewOperationObject new operation object
...
//ParseRouterComment parse route comment
...
//ParseResponseComment parse response comment
...
//Parameter http request paramter
...
//ParseParamComment parse request paramter comment
...
//ParseAcceptComment paser http header accept comment
func (operation *OperationObject) ParseAcceptComment(commentLine string) error {
	...
	for _, a := range accepts {
		...
		case "xwfu", "application/x-www-form-urlencoded":
			operation.Consumes = append(operation.Consumes, ContentTypeXFORMURLENCODE)
		}
	}
	return nil
}

//ParseProduceComment parse produce comment
func (operation *OperationObject) ParseProduceComment(commentLine string) error {
	...
	for _, a := range produces {
		...
		case "xwfu", "application/x-www-form-urlencoded":
			operation.Produces = append(operation.Produces, ContentTypeXFORMURLENCODE)
		}
	}
	return nil
}
...
//IsBasicTypeSwaggerType 是否是基本的swagger类型
...
func (operation *OperationObject) registerType(typeName string) (string, error) {
	...
	if translation, ok := typeDefTranslations[typeName]; ok {
		...
		innerModels, err := model.ParseModel(typeName, operation.parser.CurrentPackage, knownModelNames)
...
//NewModelProperty new model property
func NewModelProperty() *ModelProperty {
	return &ModelProperty{}
}

//ModelPropertyItems model property items
...
func (m *Model) ParseModel(modelName string, currentPackage string, knownModelNames map[string]bool) ([]*Model, error) {
	...
	if astTypeDef, ok := astTypeSpec.Type.(*ast.Ident); ok {
		...
		for typeName, _ := range usedTypes {
			...
			typeInnerModels, err := typeModel.ParseModel(typeName, modelPackage, knownModelNames)
			if err != nil {
...
//ParseFieldList paser field list
...
//ParseModelProperty parse model property
...
//GetTypeAsString get type as string
